...
void initUSART1(){
	...
	uint16_t BAUDRATE1 = 64UL * F_CPU / (16UL * BAUDr);	//calculate aysynchronous baudrate
	PORTMUX.USARTROUTEA = PORTMUX_USART1_DEFAULT_gc;	//USART2 using PF4 as TxD and PF5 as RxD
	PORTC.DIRSET = (1 << 0);    //Set PF4 as output (TxD)
	PORTC.DIRCLR = (1 << 1);	//Set PF5 as input (RxD)

	USART1.BAUD = BAUDRATE1;
