...
ISR(TCA0_OVF_vect) {
	...
	*/
	time1++;
	//pin_check = check_adc_within_range(0, 1);
	if(time1 > 1)
		//pin_check = check_adc_within_range(0, 1);
		setOutput(1);
		setHigh(1);
		PORTG_OUTSET = 0x01; //turn off
	if (time1 > 6){
		setLow(1);
		PORTG_OUTCLR = 0x01;
		time1 = 0;
...
{   
	...
	bool test;
	characterize(0, 1);
	//test = !check_adc_within_range(0, 1)
    ...
    {       
		...
		///*
// 		if (!pin_check) {
// 			//PORTG_OUTSET = 0x01; //turns relay off
// 			PORTG_OUTCLR = 0x01; //turns relay on
// 		}
// 		else {
// 			//PORTG_OUTCLR = 0x01; //turns relay on
// 			PORTG_OUTSET = 0x01; //turns relay off
