...
static void _FAT_freset(SD_FILE* file_p);
static void _FAT_removeChain(CLSTSIZE_t cluster);
static FRESULT _FAT_updateFileInfo(SD_FILE* fp, uint8_t task);
...
static uint8_t _FAT_nextFileCluster(SD_FILE* file_p);
...
_______________________________________________________________________________________________*/
FRESULT FAT_openDirByIndex(DIR* dir_p, SD_FILE* finfo_p, uint16_t idx){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_findByIndex(DIR* dir_p, SD_FILE* finfo_p, uint16_t idx){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_findNext(DIR* dir_p, SD_FILE* finfo_p){
	uint16_t idx;
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteFloat(SD_FILE* fp, float float_nr, uint8_t decimals){
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteInt(SD_FILE* fp, INT_SIZE nr){
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteString(SD_FILE* fp, const char* string){
	uint16_t bw;
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwrite(SD_FILE* fp, const void* buff, uint16_t btw, uint16_t* bw){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_ftruncate(SD_FILE* fp){
...
_______________________________________________________________________________________________*/
FRESULT FAT_fopen(DIR* dir_p, SD_FILE* file_p, char *file_name){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_fopenByIndex(DIR* dir_p, SD_FILE* file_p, uint16_t idx){
	FRESULT res;
	file_p->file_open = false;
	if(dir_p->dir_open == false) return FR_DENIED;
...
_______________________________________________________________________________________________*/
uint8_t* FAT_fread(SD_FILE* file_p){
	FRESULT res;
	uint16_t idx;
...
_______________________________________________________________________________________________*/
FSIZE_t FAT_getFptr(SD_FILE* fp){
	return fp->fptr;
}
...
_______________________________________________________________________________________________*/
void FAT_fseek(SD_FILE* fp, FSIZE_t fptr){	
...
_______________________________________________________________________________________________*/
void FAT_fclear_error(SD_FILE* fp){
	fp->file_err = 0;
}
...
_______________________________________________________________________________________________*/
FSIZE_t FAT_getFileSize(SD_FILE* finfo_p){
	return finfo_p->file_size;
}



uint16_t FAT_getWriteYear(SD_FILE* finfo_p){
	return 1980 + (finfo_p->file_write_date >> 9);
}


uint8_t FAT_getWriteMonth(SD_FILE* finfo_p){
	return (finfo_p->file_write_date >> 5) & 0x0F;
}


uint8_t FAT_getWriteDay(SD_FILE* finfo_p){
	return (finfo_p->file_write_date) & 0x1F;
}


uint8_t FAT_getWriteHour(SD_FILE* finfo_p){
	return (finfo_p->file_write_time >> 11) & 0x1F;
}


uint8_t FAT_getWriteMinute(SD_FILE* finfo_p){
	return (finfo_p->file_write_time >> 5) & 0x3F;
}


uint8_t FAT_getWriteSecond(SD_FILE* finfo_p){
	// << 1 is similar to * 2
...
bool FAT_attrIsFolder(SD_FILE* finfo_p){
	return finfo_p->file_attrib & FAT_FILE_ATTR_DIRECTORY;
}

bool FAT_attrIsFile(SD_FILE* finfo_p){
	return (!(finfo_p->file_attrib & FAT_FILE_ATTR_DIRECTORY));
}

bool FAT_attrIsHidden(SD_FILE* finfo_p){
	return (finfo_p->file_attrib & FAT_FILE_ATTR_HIDDEN);
}

bool FAT_attrIsSystem(SD_FILE* finfo_p){
	return (finfo_p->file_attrib & FAT_FILE_ATTR_SYSTEM);
}

bool FAT_attrIsReadOnly(SD_FILE* finfo_p){
	return (finfo_p->file_attrib & FAT_FILE_ATTR_READ_ONLY);
}

bool FAT_attrIsArchive(SD_FILE* finfo_p){
